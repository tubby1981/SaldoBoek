...
class Categorizer:
    ...
    def categorize(self, naam, omschrijving):
        ...
        logger.debug("Categoriseren: naam='%s', omschrijving='%s'", naam, omschrijving)
    ...
    def _recategorize_transactions(self, gebruiker_id):
        ...
        logger.info("Start hercategorisatie voor gebruiker %s", gebruiker_id)
        ...
        df = self.db.query_df(
            query, (categorie, self.gebruiker_id)
        )  # Veronderstel dat `db.query_df` een pandas DataFrame retourneert
        ...
        for _, row in df.iterrows():
            ...
            if nieuwe_categorie and nieuwe_categorie != oude_categorie:
                ...
                hercategoriseerd += 1
                print(
                    f"  {row['datum']} | {omschrijving[:30]:30} | {oude_categorie} → {nieuwe_categorie}"
                )
    ...
    def categoriseer_bestaande_ongecategoriseerde_transacties(self, gebruiker_id):
        ...
        df = self.db.execute_df(
            "SELECT * FROM transacties WHERE categorie = 'Ongecategoriseerd' AND gebruiker_id = ?",
            (self.gebruiker_id,),
        )
        ...
        for _, row in df.iterrows():
            ...
            if row["bedrag"] > 0:
                relevante_categorieën = [
                    c for c in alle_categorieën if c[1] == "inkomsten"
                ]
                print(f"\nBeschikbare INKOMSTEN categorieën:")
            else:
                relevante_categorieën = [
                    c for c in alle_categorieën if c[1] == "uitgaven"
                ]
                print(f"\nBeschikbare UITGAVEN categorieën:")

            for i, (cat_naam, cat_type, cat_desc) in enumerate(
                relevante_categorieën, 1
            ):
                print(f"{i:2d}. {cat_naam} ({cat_type})")
