    ...
    def parse_csv(self, filepath, account_type=None):
        ...
        try:
            ...
            beschikbare_kolommen = [
                col for col in self.relevante_kolommen if col < len(df.columns)
            ]

            if len(beschikbare_kolommen) < 6:
                print(
                    f"Waarschuwing: {filepath} heeft niet genoeg kolommen ({len(df.columns)} gevonden)"
                )
                return pd.DataFrame()
    ...
    def _ask_account_type(self, filepath):
        ...
        while True:
            ...
            if keuze == "b":
                ...
                print(
                    "Ongeldige invoer. Kies 'b' voor betaalrekening of 's' voor spaarrekening."
                )
